...
conda create python=3.12 --prefix ${ENVPREFIX} --override-channels \
           --channel conda-forge \
...
LOCAL_WHEEL_LOC=/lus/flare/projects/datasets/wheelforge/pytorch_2p8_oneapi_2025p1p3_python_3p12
...
pip install --no-deps --no-cache-dir --force-reinstall $LOCAL_WHEEL_LOC/oneccl_bind_pt-*.whl
pip install --no-deps --no-cache-dir --force-reinstall $LOCAL_WHEEL_LOC/torchvision-*.whl

#pip install --no-deps --no-cache-dir --force-reinstall torchvision==0.22.0.dev20250513+xpu --index-url https://download.pytorch.org/whl/nightly/
