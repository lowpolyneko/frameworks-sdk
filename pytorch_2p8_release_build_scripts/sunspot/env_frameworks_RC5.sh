#!/bin/bash -x
# This environment with RC4 tag tries to pin mlflow==3.3.1, it might fix an issue
# with CosmicTagger-latest to run. Needs to check if torchtune behaves normally
# Time Stamp
tstamp() {
     date +"%Y-%m-%d-%H%M%S"
}
...
CONDA_ENV_NAME=frameworks_2025.2.0_RC5
...
TMP_WORK=/lus/tegu/projects/datasets/software/wheelforge/envs/conda_envs
cd $TMP_WORK

LOCAL_WHEEL_LOC=/lus/tegu/projects/datasets/software/wheelhouse
TRY_WHEEL_LOC=${LOCAL_WHEEL_LOC}/try_wheels
...
pip install --no-cache-dir ${LOCAL_WHEEL_LOC}/pytorch_triton_xpu-*.whl
...
echo ""
echo "Finished installing the environment: frameworks/2025.2.0_RC5"
echo ""
...
pip install --no-deps --no-cache-dir --force-reinstall $TRY_WHEEL_LOC/torchtune-*.whl
pip uninstall mlflow-tracing -y
