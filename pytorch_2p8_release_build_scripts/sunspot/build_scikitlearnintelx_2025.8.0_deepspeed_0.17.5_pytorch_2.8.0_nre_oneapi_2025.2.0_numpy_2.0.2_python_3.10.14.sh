...
CONDA_ENV_NAME=scikitlearnintelx_2025.8.0_pytorch_2.8.0_deepspeed_0.17.5_nre_oneapi_2025.2.0_numpy_2.0.2_python3p10p14
...
TMP_WORK=/lus/tegu/projects/datasets/software/wheelforge/repositories/intelex_scikitlearn_08_22_2025
cd $TMP_WORK
...
python setup.py bdist_wheel --dist-dir ${TMP_WORK}/${CONDA_ENV_NAME} 2>&1 | tee ${TMP_WORK}/${CONDA_ENV_NAME}/"intelxscikitlearn-build-whl-$(tstamp).log"

echo ""
echo "Completed build scikit-learn-intelx/2025.8.0 wheel with mpi4py/4.1.0 and oneapi/2025.2.0 with numpy/2.0.2 for PyTorch-2.8.0 with IPEX-2.8.10" 
echo ""

INTELXSCIKITLEARN_WHEEL_LOC=${TMP_WORK}/${CONDA_ENV_NAME}
pip install --no-deps --no-cache-dir --force-reinstall $INTELXSCIKITLEARN_WHEEL_LOC/scikit_learn_intelex-*.whl 2>&1 | tee ${TMP_WORK}/${CONDA_ENV_NAME}/"intelxscikitlearn-install-$(tstamp).log"
echo ""
