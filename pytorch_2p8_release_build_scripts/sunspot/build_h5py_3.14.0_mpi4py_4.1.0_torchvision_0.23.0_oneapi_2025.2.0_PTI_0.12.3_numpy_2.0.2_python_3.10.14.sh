...
CONDA_ENV_NAME=h5py_3.14.0_mpi4py_4.1.0_torchvision_0.23.0_oneapi_2025.2.0_pti_0.12.3_numpy_2.0.2_python3p10p14
...
TMP_WORK=/lus/tegu/projects/datasets/software/wheelforge/repositories/h5py_3.14.0_rel_08_19_2025
cd $TMP_WORK

LOCAL_WHEEL_LOC=/lus/tegu/projects/datasets/software/wheelhouse

pip install --no-cache-dir -r ${LOCAL_WHEEL_LOC}/h5py_3.14.0_torchvision_0.23.0_ipex_2.8.10_pytorch_2.8.0_triton_xpu_3.4.0_combined_requirements.txt
...
#git clone git clone git@github.com:h5py/h5py.git
cd h5py/
#git checkout 3.14.0
...
echo ""
echo "Completed build h5py/3.14.0 wheel with mpi4py/4.1.0 and  oneapi/2025.2.0 with numpy/2.0.2 with CXX = $(which mpicxx) and CC = $(which mpicc)" 
echo ""

H5PY_WHEEL_LOC=${TMP_WORK}/${CONDA_ENV_NAME}
pip install --no-deps --no-cache-dir --force-reinstall $H5PY_WHEEL_LOC/h5py-*.whl 2>&1 | tee ${TMP_WORK}/${CONDA_ENV_NAME}/"h5py-install-$(tstamp).log"
h5cc -showconfig
echo ""
echo "Finished installing the h5py/3.14.0 wheel and dependencies"
echo ""
