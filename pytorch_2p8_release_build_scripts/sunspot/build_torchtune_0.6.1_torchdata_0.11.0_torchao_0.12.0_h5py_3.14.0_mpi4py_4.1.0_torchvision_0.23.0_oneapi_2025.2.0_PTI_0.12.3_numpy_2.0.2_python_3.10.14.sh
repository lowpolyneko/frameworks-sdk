...
TMP_WORK=/lus/tegu/projects/datasets/software/wheelforge/repositories/torchtune_0.6.1_rel_08_20_2025
cd $TMP_WORK
...
pip wheel --no-deps --wheel-dir ${TMP_WORK}/${CONDA_ENV_NAME} -v -e '.[dev]' 2>&1 | tee ${TMP_WORK}/${CONDA_ENV_NAME}/"torchtune-build-whl-$(tstamp).log"

echo ""
echo "Completed build torchtune/0.6.1 wheel with mpi4py/4.1.0 and oneapi/2025.2.0 with numpy/2.0.2 for PyTorch-2.8.0 with IPEX-2.8.10" 
echo ""

TORCHTUNE_WHEEL_LOC=${TMP_WORK}/${CONDA_ENV_NAME}
pip install --no-deps --no-cache-dir --force-reinstall $TORCHTUNE_WHEEL_LOC/torchtune-*.whl 2>&1 | tee ${TMP_WORK}/${CONDA_ENV_NAME}/"torchtune-install-$(tstamp).log"
echo ""
echo "Finished installing the torctune/0.6.1 wheel and dependencies"
echo ""
